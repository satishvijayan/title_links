$(function() {
	const allowedRoutes = ["Form/Address"];
	...
	const checkRoute = () => {
		...
		while (i < allowedRoutes.length && !routeFound) {
			if (window.location.hash.includes(allowedRoutes[i])) {
	...
	$(window).bind('hashchange', (e) => {
        setTimeout(() => {
			const isCurrentRouteAllowed = checkRoute();
			// console.log("Hash changed", isCurrentRouteAllowed, isLinksHasBeenFormated);

			if (!isCurrentRouteAllowed && isLinksHasBeenFormated) { // remove formatLinks(); effect on non allowed routes
				location.reload();
			}

			if (isCurrentRouteAllowed && isLinksHasBeenFormated) { // formatLinks(); works on reload (ps: I don't know why the page needs to be reloaded first)
				location.reload();
			}
        }, 1000);
	});
